...
- [`AUTHORING_UX_PLAN.md`](AUTHORING_UX_PLAN.md) — orbit-while-drafting, the grazing guard and the shared keyboard map (Phase A) and the snapping pen with undo, nudge, loupe and mirror (Phase B), and landmark placement by drag with a guided order, mirror offers and `placed_with` records carried through both engines (Phase C), all built; template drafts from landmarks (Phase D) planned.
