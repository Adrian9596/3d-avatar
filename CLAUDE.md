...
- `npm run validate:measurements` — chains all of the above in order (sync, measure, then the six gates). Run it after any re-export. Each gate is also its own npm script, so run one alone when iterating.
...
- **Flattening (2D pattern draft) has one start and one objective.** `scripts/flatten_core.mjs` lays a patch out by hinge unfolding (exact on a developable surface) and then relaxes it with boundary edges held hard to their 3D length and the interior yielding — because a pattern piece is judged at its seam, not in its middle. Do not add a conformal (LSCM) or ARAP stage in front of it: measured on this body they change nothing to 0.01mm and would need a sparse solver the stdlib-only Python port cannot have. Do not rescale a piece to hide its boundary error; report it. The pen loop → patch step (`extractPatch`) overshoots the drawn loop by up to one triangle and does not yet hold the loop itself to length — a declared limit, listed in `qa/avatar_master/flatten-accuracy.json`. Nothing here is a pattern: no ease, no seam allowance, no grading, no DXF yet (`PATTERN_2D_DXF_PLAN.md` §11–13).
