...
- `npm run validate:measurements` — chains all of the above in order (sync, measure, then the eight gates). Run it after any re-export. Each gate is also its own npm script, so run one alone when iterating.
...
- **The two lanes differ in role, not in maths.** `digital_bra_fit_model_360.html` is the authoring lane (pen, hand-placed landmarks, live section, and the 2D pattern draft: a closed pen loop is an outline, an open pen line whose ends sit on it is a seam, Flatten runs `flattenPieces`, Export DXF writes the ASTM/Gerber file); `viewer/` is read-only presentation and `validate:lane-parity` checks it imports neither the flattening engine nor the DXF writer. Both import `scripts/measure_core.mjs` and read the same registry, so they cannot disagree about a number — `npm run validate:lane-parity` enforces that. Do not add landmark editing to the production lane: one place to correct, one record.
